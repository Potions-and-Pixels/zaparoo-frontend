...
TestCase {
    ...
    function test_font_size_respects_minimum_for_240p(): void {
        ...
        verify(Sizing.fontSize(2) >= 8,
               "fontSize must never fall below 8px for CRT legibility")
    ...
    function test_sizing_updates_propagate_proportionally(): void {
        ...
        verify(Math.abs(scaled - baseline * 1.5) <= 1,
               "pctH scaling should track screen height proportionally")
    }
}
